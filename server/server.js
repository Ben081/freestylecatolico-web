...
app.post('/api/donaciones', (req, res) => {
  try {
    const { proyecto, nombre, correo, monto, fee = 0, anonimo = false, fuente = 'desconocida' } = req.body
    ...
    const stmt = db.prepare(`
      INSERT INTO donaciones (proyecto, nombre, correo, monto, comision, monto_neto, fee, anonimo, fuente)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
...
app.post('/api/convocatoria', async (req, res) => {
  try {
    const { nombre, correo, nivelEstudios, interes, mensaje, mayorEdad } = req.body

    if (!nombre || !correo || !nivelEstudios || !mayorEdad) {
      ...
      <div style="font-family: 'DM Sans', sans-serif; max-width: 560px; margin: 0 auto; padding: 32px; background: #0f0f12; border-radius: 12px; border: 1px solid rgba(201,162,39,0.3);">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            ...
            <td style="padding: 8px 0; color: #f3ead3; font-size: 14px;">${interes || 'No especificado'}</td>
          </tr>
          <tr>
        ...
        <p style="color: #948d9e; font-size: 11px; margin: 0;">Freestyle Católico 2026 — Programa Itinerante de Coaching Corporativo</p>
      </div>
    `

    await transporter.sendMail({
      from: `"Freestyle Católico" <${process.env.SMTP_USER}>`,
...
app.get('/api/donaciones/csv', requireAdmin, (req, res) => {
  try {
    ...
    const header = 'ID,Proyecto,Nombre,Correo,Monto,Comisión,Monto Neto,Fee,Anónimo,Fuente,Fecha'
    const rows = donaciones.map(d =>
      [d.id, d.proyecto, `"${d.nombre}"`, d.correo || '', d.monto, d.comision, d.monto_neto, d.fee, d.anonimo ? 'Sí' : 'No', d.fuente, d.creado_en].join(',')
    )
